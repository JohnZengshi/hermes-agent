...
#
# strict (default): least-privilege sandbox.
# relaxed: allows agent-driven environment bootstrap (e.g. apt install)
#          by adding SETUID/SETGID and removing no-new-privileges.
def _build_security_args(security_mode: str = "strict") -> list[str]:
    mode = (security_mode or "strict").strip().lower()
    if mode not in {"strict", "relaxed"}:
        logger.warning(
            "Unknown docker security_mode %r, falling back to 'strict'", security_mode
        )
        mode = "strict"
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        security_args = _build_security_args(security_mode)
        all_run_args = (
            list(security_args) + writable_args + resource_args + volume_args + env_args
        )
