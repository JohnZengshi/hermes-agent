...
class TestDeviceContextExtraction:
    def test_extracts_device_context_from_headers(self):
        request = MagicMock()
        request.headers = {
            "X-Hermes-Device-Platform": "ios",
            "X-Hermes-Device-OS-Version": "18.1",
            "X-Hermes-Client-Name": "Kelivo",
            "User-Agent": "Kelivo",
        }
    ...
    def test_extracts_device_context_from_body_and_metadata(self):
        request = MagicMock()
        request.headers = {}

        context = _extract_device_context(
            request,
            {
                "metadata": {
                    "device": {
...
class TestChatCompletionsEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_device_headers_do_not_change_system_prompt(self, adapter):
        mock_result = {
            "final_response": "I am a pirate! Arrr!",
            "messages": [],
            "api_calls": 1,
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert call_kwargs.get("ephemeral_system_prompt") == "You are a pirate."
