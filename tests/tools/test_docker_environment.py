...
def _make_dummy_env(**kwargs):
    ...
    return docker_env.DockerEnvironment(
        ...
        env=kwargs.get("env"),
        security_mode=kwargs.get("security_mode", "strict"),
    )
...
def test_security_mode_relaxed_adds_setuid_setgid_and_omits_nnp(monkeypatch):
    """relaxed mode should allow set*id flows (apt user/group ops)."""
    monkeypatch.setattr(docker_env, "find_docker", lambda: "/usr/bin/docker")
    calls = _mock_subprocess_run(monkeypatch)
    ...
    assert "--security-opt no-new-privileges" in run_str

    calls.clear()
    _make_dummy_env(security_mode="relaxed")
    run_calls = [
        c
        for c in calls
        if isinstance(c[0], list) and len(c[0]) >= 2 and c[0][1] == "run"
    ...
    run_args = run_calls[0][0]
    run_str = " ".join(run_args)
    assert "--cap-add SETUID" in run_str
    assert "--cap-add SETGID" in run_str
    assert "--security-opt no-new-privileges" not in run_str
