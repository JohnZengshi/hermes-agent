        ...
        if not skip_memory:
            try:
                ...
                if self._memory_enabled or self._user_profile_enabled:
                    ...
                    memory_backend_name = str(mem_config.get("backend", "file")).lower()
                    if memory_backend_name == "sqlite":
                        memory_backend = SQLiteMemoryBackend(user_id=self._user_id)
                    else:
                        memory_backend = FileMemoryBackend(user_id=self._user_id)
                    ...
                    logger.info(
                        "Memory initialized: store=%s backend=%s user_id=%s entries(memory=%d,user=%d) "
                        "nudge_interval=%d",
                        type(self._memory_store).__name__,
                        type(self._memory_store._backend).__name__,
                        self._user_id or "(none)",
                        len(self._memory_store.memory_entries),
                        len(self._memory_store.user_entries),
