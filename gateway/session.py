...
class SessionContext:
    ...
    # Session isolation policy (copied from GatewayConfig at context build time)
...
def build_session_context_prompt(
    ...
    *,
    ...
    # User identity.
    # In shared non-DM sessions, multiple users contribute to the same
    ...
    _is_non_dm = context.source.chat_type != "dm"
    _is_shared_thread = (
        _is_non_dm
        and bool(context.source.thread_id)
        and not context.thread_sessions_per_user
    )
    _is_shared_non_thread_group = (
        _is_non_dm
        and not context.source.thread_id
        and not context.group_sessions_per_user
    )
    _is_shared_multi_user_session = _is_shared_thread or _is_shared_non_thread_group

    if _is_shared_multi_user_session:
        lines.append(
