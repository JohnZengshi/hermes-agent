...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def edit_message(
        ...
        try:
            ...
            if "message_too_long" in err_str or "too long" in err_str:
                truncated = (
                    _prefix_within_utf16_limit(content, self.MAX_MESSAGE_LENGTH - 20)
                    + "…"
                )
                try:
                    await self._bot.edit_message_text(
    ...
    async def _handle_command(
        ...
        if not update.message or not update.message.text:
            return
        if not self._command_targets_this_bot(update.message.text):
            return
    ...
    def _command_targets_this_bot(self, text: Optional[str]) -> bool:
        """Return True when a slash command targets this bot (or no target specified).

        Telegram supports `/cmd@bot_username ...` in group chats. Without this
        check, commands aimed at another bot can be processed here because the
        shared parser later strips the `@bot_username` suffix.
        """
        if not text:
            return False
        ...
        bot_username = (
            (getattr(self._bot, "username", None) or "").strip().lstrip("@").lower()
        )

        # If we cannot determine our own username, don't block command handling.
        if not bot_username:
            return True
        return bool(target) and target == bot_username
