...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._router_config: Dict[str, Any] = self._parse_router_config(
            extra.get("router")
        )
        self._router_session: Optional[ClientSession] = None
    ...
    @staticmethod
    def _parse_router_config(value: Any) -> Dict[str, Any]:
        """Normalize optional router mode config."""
        if not isinstance(value, dict):
            return {"enabled": False, "default_backend": None, "backends": {}}
        ...
        for alias, backend in raw_backends.items():
            if (
                ...
                continue
            backend_alias = alias.strip()
            model_id = str(backend.get("model_id", "")).strip()
            base_url = str(backend.get("base_url", "")).strip().rstrip("/")
            if not model_id or not base_url:
                continue
            if model_id in model_ids:
                raise ValueError(f"Duplicate router model_id configured: {model_id}")
        ...
        default_backend = value.get("default_backend")
        default_alias = (
            str(default_backend).strip() if default_backend is not None else None
        )
        if default_alias and default_alias not in backends:
            raise ValueError(f"Unknown router default_backend: {default_alias}")

        return {
            "enabled": bool(backends),
            "default_backend": default_alias,
            "backends": backends,
        }

    def _router_enabled(self) -> bool:
        return bool(self._router_config.get("enabled"))

    def _router_backends(self) -> Dict[str, Dict[str, str]]:
        return self._router_config.get("backends", {})

    def _router_default_backend(self) -> Optional[Dict[str, str]]:
        alias = self._router_config.get("default_backend")
        if not alias:
            return None
        return self._router_backends().get(alias)

    def _get_backend_by_alias(self, alias: Optional[str]) -> Optional[Dict[str, str]]:
        if not alias:
            return None
        return self._router_backends().get(alias)

    def _get_backend_by_model(
        self, model_id: Optional[str]
    ) -> Optional[Dict[str, str]]:
        if not model_id:
            return None
        for backend in self._router_backends().values():
            if backend["model_id"] == model_id:
                return backend
        return None

    def _make_public_token(self, backend_alias: str, raw_value: str) -> str:
        encoded = (
            base64.urlsafe_b64encode(raw_value.encode("utf-8"))
            .decode("ascii")
            .rstrip("=")
        )
        return f"{backend_alias}__{encoded}"

    def _parse_public_token(
        self, public_value: Optional[str], token_kind: str
    ) -> tuple[Optional[Dict[str, str]], Optional[str], Optional[Dict[str, Any]]]:
        if not public_value:
            return None, None, None
        if "__" not in public_value:
            return None, None, _openai_error(f"Invalid {token_kind}: {public_value}")
        alias, encoded = public_value.split("__", 1)
        backend = self._get_backend_by_alias(alias)
        if backend is None:
            return (
                None,
                None,
                _openai_error(f"Unknown backend for {token_kind}: {public_value}"),
            )
        if not encoded:
            return None, None, _openai_error(f"Invalid {token_kind}: {public_value}")
        ...
        *hints: tuple[str, Optional[Dict[str, str]]],
        ...
        for source, backend in hints:
            ...
            if selected is None:
                ...
                continue
            if selected["alias"] != backend["alias"]:
                return None, _openai_error(
                    f"Conflicting routing hints: {selected_source} maps to '{selected['model_id']}' but {source} maps to '{backend['model_id']}'"
                )
        return selected, None

    async def _ensure_router_session(self) -> ClientSession:
        if self._router_session is None or self._router_session.closed:
            self._router_session = ClientSession(timeout=ClientTimeout(total=None))
    ...
    def _rewrite_router_response_payload(
        self, backend: Dict[str, str], payload: Any
    ) -> Any:
        if not isinstance(payload, dict):
            return payload
        rewritten = json.loads(json.dumps(payload))
        if isinstance(rewritten.get("id"), str):
            rewritten["id"] = self._make_public_token(backend["alias"], rewritten["id"])
        if isinstance(rewritten.get("run_id"), str):
            rewritten["run_id"] = self._make_public_token(
                backend["alias"], rewritten["run_id"]
            )
        return rewritten

    def _rewrite_router_sse_line(self, backend: Dict[str, str], line: bytes) -> bytes:
        prefix = b"data: "
        ...
        if isinstance(event, dict) and isinstance(event.get("run_id"), str):
            event["run_id"] = self._make_public_token(backend["alias"], event["run_id"])
            return prefix + json.dumps(event).encode("utf-8") + b"\n"
        return line

    async def _router_proxy_json(
        self,
        request: "web.Request",
        backend: Dict[str, str],
        *,
        ...
        session = await self._ensure_router_session()
        headers = self._build_router_headers(request, backend, session_id=session_id)
        async with session.request(
            method, f"{backend['base_url']}{path}", json=json_body, headers=headers
        ) as resp:
            ...
            return web.json_response(
                payload, status=resp.status, headers=response_headers
            )

    async def _router_proxy_stream(
        self,
        request: "web.Request",
        backend: Dict[str, str],
        *,
        ...
        response_headers = {
            ...
            "Cache-Control": backend_response.headers.get("Cache-Control", "no-cache"),
            "X-Accel-Buffering": backend_response.headers.get(
        ...
        response = web.StreamResponse(
            status=backend_response.status, headers=response_headers
        )
        await response.prepare(request)
        try:
            async for line in backend_response.content:
                if rewrite_run_events:
                    line = self._rewrite_router_sse_line(backend, line)
                await response.write(line)
        finally:
            backend_response.release()
        return response

    def _resolve_router_backend(
        self,
        *,
        ...
        Optional[Dict[str, str]], Optional[str], Optional[str], Optional[Dict[str, Any]]
    ]:
        session_backend = None
        session_raw = None
        if session_token:
            session_backend, session_raw, error = self._parse_public_token(
        ...
        if previous_response_id:
            previous_backend, previous_raw, error = self._parse_public_token(
        ...
        if conversation and not previous_raw:
            mapped = self._response_store.get_conversation(conversation)
            if mapped:
                conversation_backend, conversation_raw, error = (
                    self._parse_public_token(mapped, "conversation response id")
                )
                if error:
                    return None, None, None, error

        model_backend = self._get_backend_by_model(model) if model else None
        if model and model_backend is None:
            return (
                None,
                None,
                None,
                _openai_error(f"Unsupported model: {model}", code="model_not_found"),
            )

        backend, error = self._merge_routing_hints(
            ("session", session_backend),
            ("previous_response_id", previous_backend),
        ...
        if error:
            return None, None, None, error
        if backend is None:
            backend = self._router_default_backend()
        if backend is None:
            return (
                None,
                None,
                None,
                _openai_error("Unable to resolve backend for request"),
            )

        raw_previous = previous_raw or conversation_raw
        return backend, session_raw, raw_previous, None
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        if self._router_enabled():
            return web.json_response(
                {
                    "status": "ok",
                    "platform": "hermes-router",
                    "backends": [
                        {"alias": backend["alias"], "model": backend["model_id"]}
                        for backend in self._router_backends().values()
                    ],
                }
            )
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        ...
        if self._router_enabled():
            model_ids = [
                backend["model_id"] for backend in self._router_backends().values()
            ]
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if self._router_enabled():
            ...
            if body.get("stream", False):
                return await self._router_proxy_stream(
                    request,
                    backend,
                    method="POST",
            ...
            return await self._router_proxy_json(
                request,
                backend,
                method="POST",
                path="/v1/chat/completions",
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        if self._router_enabled():
            ...
            backend, _, raw_previous_response_id, routing_error = (
                self._resolve_router_backend(
                    model=body.get("model"),
            ...
            if raw_previous_response_id:
                proxied_body["previous_response_id"] = raw_previous_response_id
            response = await self._router_proxy_json(
                request,
                backend,
                method="POST",
                path="/v1/responses",
                json_body=proxied_body,
            )
            if (
                response.status < 400
    ...
    async def _handle_get_response(self, request: "web.Request") -> "web.Response":
        ...
        if self._router_enabled():
            backend, raw_response_id, error = self._parse_public_token(
                request.match_info["response_id"], "response_id"
            )
            if error:
            ...
            return await self._router_proxy_json(
                ...
                backend,
                method="GET",
                path=f"/v1/responses/{raw_response_id}",
            )
    ...
    def _check_jobs_available(self) -> Optional["web.Response"]:
        ...
        if self._router_enabled():
            return web.json_response(
                {"error": "Cron jobs API not available in router mode"}, status=501
            )
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        if self._router_enabled():
            backend, raw_session_id, raw_previous_response_id, routing_error = (
                self._resolve_router_backend(
                    model=body.get("model"),
            ...
            proxied_body = dict(body)
            if raw_session_id:
                proxied_body["session_id"] = raw_session_id
            if raw_previous_response_id:
                proxied_body["previous_response_id"] = raw_previous_response_id
            return await self._router_proxy_json(
                request,
    ...
    async def _handle_run_events(self, request: "web.Request") -> "web.StreamResponse":
        ...
        if self._router_enabled():
            backend, raw_run_id, error = self._parse_public_token(
            ...
            return await self._router_proxy_stream(
                ...
                backend,
                method="GET",
                path=f"/v1/runs/{raw_run_id}/events",
                rewrite_run_events=True,
            )
    ...
    async def disconnect(self) -> None:
        ...
        if self._router_session is not None:
            await self._router_session.close()
